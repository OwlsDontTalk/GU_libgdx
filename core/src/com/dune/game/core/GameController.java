...
public class GameController {
    ...
    public void checkHit(float dt){
        for(int i = 0; i < projectilesController.activeSize(); i++){
            Projectile p = projectilesController.activeList.get(i);
            for(int j = 1; j < tanksController.activeSize(); j++) {
                Tank t = tanksController.activeList.get(j);
                if( Math.abs(t.position.x - p.position.x) < 30 && Math.abs(t.position.y - p.position.y) < 30 && t.getOwnerType() == Tank.Owner.AI){
                    p.deactivate();
                    t.getHit();
                }
            }
